...
struct matrix * make_rotX(double theta) {
  ...
  struct matrix * temp = new_matrix(4,4);
  ident(temp);
  double degrees = theta * M_PI / 180;

  temp->m[0][0] = cos(degrees);
  temp->m[0][1] = -sin(degrees);
  temp->m[1][0] = sin(degrees);
  temp->m[1][1] = cos(degrees);
...
struct matrix * make_rotY(double theta) {
  ...
  temp->m[0][0] = cos(degrees);
  temp->m[0][2] = sin(degrees);
  temp->m[2][0] = -sin(degrees);
  temp->m[2][2] = cos(degrees);
